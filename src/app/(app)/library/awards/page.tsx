...
export default async function AwardsPage({ searchParams }: { searchParams: Record<string, string | string[] | undefined> }) {
  ...
  const [context, data, refRates] = await Promise.all([getSessionWorkspace(), loadAwards(supabase, parseAwardsFilters(searchParams), today), supabase.from("reference_success_rates").select("id, mechanism, fiscal_year, rate, label, source_url").order("mechanism").order("fiscal_year", { ascending: false })]);
