...
export async function saveOpportunitiesAction(input: { opportunityIds: string[]; saved: boolean }): Promise<Result<{ changed: number }>> {
  const ids = z.array(uuid).min(1).max(500).safeParse(input.opportunityIds);
  if (!ids.success) return { ok: false, error: "Nothing selected." };
  const guard = await requireTeamRole("member");
  if (!guard.ok) return guard;
  ...
  if (!input.saved) {
    const { error } = await admin.from("saved_funding_opportunities").delete().eq("team_id", actor.teamId).in("opportunity_id", ids.data);
    if (error) return { ok: false, error: error.message };
    revalidateOpportunities();
    return { ok: true, changed: ids.data.length };
  }

  // One row per team and notice; the legacy key still carries the saver's id.
  const { data: existing } = await admin
    ...
    .in("opportunity_id", ids.data);
  const have = new Set(((existing ?? []) as Array<{ opportunity_id: string }>).map((r) => r.opportunity_id));
  const rows = ids.data.filter((id) => !have.has(id)).map((opportunity_id) => ({ team_id: actor.teamId, user_id: actor.userId, opportunity_id }));
  if (rows.length) {
    const { error } = await admin.from("saved_funding_opportunities").insert(rows);
    if (error) return { ok: false, error: error.message };
  }
