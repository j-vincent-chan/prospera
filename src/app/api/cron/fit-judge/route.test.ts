...
const URL = "http://localhost/api/cron/fit-judge";
const auth = { authorization: "Bearer test-secret" };

/** A database without the PR 3.1 migration: fit_adjudications absent, the job log present. */
function beforeMigration(over: Partial<FakeTables> = {}) {
  return fakeDb({ sync_job_logs: [], fit_adjudications: null, fit_corrections: null, ...over });
}

describe("/api/cron/fit-judge", () => {
  beforeEach(() => {
  ...
  it("before the migration the run is skipped: 200, logged as success with outcome skipped, no model call, nothing written", async () => {
    const db = beforeMigration();
    holder.db = db;
    const res = await GET(new Request(URL, { headers: auth }));
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body).toMatchObject({ ok: true, skipped: expect.stringMatching(/fit_adjudications is not on the database/), message: expect.stringMatching(/^fit_judge skipped/) });
    const writes = db.log.writes.map((w) => `${w.table}:${w.op}`);
    expect(writes).toEqual(["sync_job_logs:insert", "sync_job_logs:update"]);
    expect(db.log.writes[0]!.rows[0]).toMatchObject({ job_type: "fit_judge", status: "started", details: { limit: null, cursor: null, max_model_calls: 150 } });
    expect(db.log.writes[1]!.values).toMatchObject({ status: "success", details: expect.objectContaining({ outcome: "skipped" }) });
  });

  it("a POST with a cursor or ids that are not investigator UUIDs is a 400 before anything runs", async () => {
    const db = beforeMigration();
    holder.db = db;
  ...
  it("a POST dry run writes no log row", async () => {
    const db = beforeMigration();
    holder.db = db;
    const res = await POST(new Request(URL, { method: "POST", headers: { ...auth, "content-type": "application/json" }, body: JSON.stringify({ dryRun: true, maxModelCalls: 0, cursor: "001f7e5b-4cda-43ff-b12a-ec8dfe428e98" }) }));
