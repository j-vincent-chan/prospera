 ...
 * edits an existing person from their profile page.
 */
...
export function InvestigatorFormSheet({
  ...
  const [showIds, setShowIds] = useState(Boolean(initial?.orcid || initial?.nih_profile_id || initial?.profiles_url_name));
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    if (open) {
      setV(initial ?? { ...EMPTY_FORM, research_community_id: defaultCommunityId ?? "" });
      setShowIds(Boolean(initial?.orcid || initial?.nih_profile_id || initial?.profiles_url_name));
      setError(null);
    }
    ...
    <SlideOver
      ...
      footer={
        <div className="flex justify-end gap-2">
          ...
          <Button variant="primary" onClick={submit} disabled={pending || !v.first_name.trim() || !v.last_name.trim()}>
      ...
      <div className="flex flex-col gap-3.5 px-6 py-5">
        ...
        <div className="border-t border-line-row pt-3">
          <button type="button" onClick={() => setShowIds((s) => !s)} aria-expanded={showIds} className="m-0 text-dense font-medium text-teal hover:text-navy">
            Identifiers (optional) — ORCID, NIH RePORTER profile ID
          </button>
          {showIds ? (
            <div className="mt-2.5 grid grid-cols-2 gap-3">
              <Input value={v.orcid} onChange={set("orcid")} placeholder="ORCID" aria-label="ORCID iD" className="font-mono text-dense" />
              ...
              <Input value={v.profiles_url_name} onChange={set("profiles_url_name")} placeholder="profiles.ucsf.edu link (optional)" aria-label="UCSF Profiles link" className="col-span-2 text-dense" />
            </div>
          ) : null}
        </div>
        {!editing ? (
