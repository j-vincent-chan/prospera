...
export async function sourceHealth(db: SupabaseClient): Promise<SourceHealth> {
  const [{ data: logs }, { count: notices }, { count: forecasted }, { count: withCycles }, { data: sources }, { count: sentWeek }, { count: failedWeek }, { count: repliesWeek }] = await Promise.all([
  ...
  const base: Array<Omit<SourceRow, "statusLabel">> = [
    ...
    { key: "osr", name: "OSR awards & declines", what: "UCSF award history and aggregated decline counts from the Office of Sponsored Research. Powers Awards and the track-record panel.", status: "not_connected", coverage: "Arrives with the institutional layer (step 7)", last: "—", next: "Requires OSR access", action: { kind: "none", label: "Connect" } },
    { key: "rap", name: "RAP (internal funding)", what: "Resource Allocation Program announcements, published to the Internal (UCSF) scope as Curated records.", status: "not_connected", coverage: "Arrives with the institutional layer (step 7)", last: "—", next: "—", action: { kind: "none", label: "Connect" } },
    { key: "infoready", name: "InfoReady (limited submissions)", what: "No API confirmed. Curators re-key competitions as overlays on synced sponsor notices.", status: "manual", coverage: "Overlays arrive with the institutional layer (step 7)", last: "—", next: "Check weekly", action: { kind: "link", label: "Open InfoReady", href: "https://ucsf.infoready4.com/" } },
