...
/**
 * A design id as a decision surface reads it.
 *
 * `taxonomy.json` gives designs no display label — only ids grouped under
 * `design.groups` — so this used to de-underscore the id, which leaves
 * `rct` as "rct", `gwas` as "gwas" and
 * `hybrid_effectiveness_implementation` as "hybrid effectiveness
 * implementation". `inspect/display-labels.ts` (#61) is the written reading of
 * each id — "Randomized controlled trial", "Genome-wide association study",
 * "Hybrid effectiveness–implementation trial" — and it is keyed by
 * `DesignId`, so a new design fails the build here until it has words. The
 * words are still never invented at the call site; they are looked up.
 *
 * Exported for PR 4's design rows.
 */
export const designWords = designLabel;

/** "Genome-wide association study or Secondary data analysis" — a required design group as one phrase. Exported for PR 4's requirements table, which names the same groups the caveat does. */
