 ...
 *
 * **`missing` is a verb here too** (fit-UX merge). `engine/methods.ts` builds
 * `M.missing` out of `CapabilityItem.label`, which is a materials kind id, or
 * a required design group joined by `" | "`, or a named piece of
 * infrastructure — and `engine/explain.ts` prints the list verbatim after the
 * word: "Methods 0.25 is below the Moderate floor 0.3; missing gwas |
 * secondary_data_analysis, cohort_biobank_datasets, genomic_datasets". That
 * clause reaches the row's caveat and the panel's bullets, so the bare ids in
 * it (`gwas`, `ehr`, `survey`) were the ones the map could not reach. An
 * infrastructure name that happens to equal an id is rewritten to the same
 * words, so nothing is falsified.
 *
 * The run therefore takes `" | "` as well as `", "`: the pipe is the engine's
 * "any of these", and reads as "or" once the ids are phrases — the same
 * reading `DESIGN_RUN_RE` gives it.
 */
const ANY_ID_ALT = Array.from(BY_ID.keys()).sort((a, b) => b.length - a.length).join("|");
const VERB_ID_RE = new RegExp(`(?<=\\b(?:prohibits|excludes|requires|allows|names|missing)\\s)((?:${ANY_ID_ALT})(?:(?:,|\\s\\|)\\s(?:${ANY_ID_ALT}))*)(?![A-Za-z0-9_-])`, "g");

/** Splits a `VERB_ID_RE` run on its separators, keeping them, so `a | b, c` comes back as ids and joins. */
const ID_RUN_SPLIT = /(,\s|\s\|\s)/;
...
export function humanizeIds(text: string | null | undefined): string | null {
  ...
  out = out.replace(VERB_ID_RE, (run) => run.split(ID_RUN_SPLIT).map((part, i) => (i % 2 ? (part === " | " ? " or " : part) : displayLabel(part))).join(""));
