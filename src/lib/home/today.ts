  ...
  const cards: TodayCard[] = [
    {
      ...
      items: decideItems,
      empty: input.fitEngine === "fit-v1" ? "Nothing is waiting for a decision. A notice enters here when someone in the directory clears the bar for it." : "Matching is on the legacy engine for this team; the PI Match queue starts when it moves to fit-v1.",
    },
    {
      key: "answer",
      title: "Answer a PI",
      sub: answerSub({ replies: replies.length, dated, since: sinceWord }),
      cta: { label: "Open PI Outreach", href: "/outreach", kind: "secondary" },
      items: answerItems,
