...
**Acceptance.** FON decomposes for ≥ 95% of the 81 rows (PR 5.0 got 79/81 on shape) with **zero unmatched mechanisms** against the seeded program list, and the distribution reported; the `_GG.pdf` route resolves at or above the **86.1%** PR 5.0 measured (§ 5c, 68/79 — an earlier "88%" here was the capped run's 44/50 and contradicted its own source); `objectives` recovered for ≥ 90%; one saved fixture; a PR note listing every distinct mechanism suffix seen, which is PR 5.8's input.
