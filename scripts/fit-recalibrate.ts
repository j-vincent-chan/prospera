 ...
 *
 * Fixture pairs: a `--labels-csv` row whose `investigator_id` is
 * `fixture:<case id>` is scored from the adversarial fixture
 * (`engine/fixtures.ts`) instead of the database, with the CSV's tier as its
 * label. That is how the §13 regression cases enter the fit — their expected
 * tiers are labels the search must not break — and how the search can be
 * exercised without strategist labels. A run whose labels are constructed is
 * marked FIXTURE RUN in the proposal and is never a calibration.
 */
...
import { computeMetrics, type MetricPair } from "../src/lib/fit/goldset/metrics";
...
import { patchPath, proposalPath, renderConsole, renderProposal, RECALIBRATION_DIR, type RecalibrationInput, type StratumLabels } from "../src/lib/fit/recalibrate/report";
...
async function main(): Promise<void> {
  ...
  // 2 · the pairs: the gold set, plus any fixture pair the CSV names.
  ...
  const fixtureIds = Array.from(new Set(Array.from(csvLabels.keys()).flatMap((k) => (k.startsWith(FIXTURE_PREFIX) ? [k] : []))));
  const fixtures = new Map(loadAdversarialCases().map((c) => [`${FIXTURE_PREFIX}${c.id}`, c]));
  let n = 0;
  for (const k of fixtureIds.sort()) {
    const [inv, opp] = k.split("|");
    n += 1;
    set.push({ id: `f${String(n).padStart(3, "0")}`, investigator_id: inv!, opportunity_id: opp!, stratum: "extra", synthetic: true, source: `fixture:${fixtureCaseOf(inv!) ?? "?"}` });
  }
  if (n) notes.push(`${n} adversarial-fixture pair(s) from \`--labels-csv\` are scored from \`engine/fixtures.ts\` (spec §13); their labels are the fixture's expected tiers, so the regression suite constrains the fit.`);
  ...
  const metricsOptions = (generated: string) => ({ forbidden, generated_at: generated, goldset_version: GOLDSET_MANIFEST.version, seed: GOLDSET_MANIFEST.seed, taxonomy_version: TAXONOMY_VERSION, engine_version: ENGINE_VERSION, source: "goldset" as const, grid: null, notes: [] as string[] });
  const before = computeMetrics(metricPairs(beforeScores), metricsOptions(nowIso));

  const labeled = pairs.filter((p) => p.label.tier !== null).length;
  ...
  const fitted: FittedPair[] = pairs.map((p) => ({ id: p.id, label: p.label.tier, forbidden: p.forbidden, legacy: p.synthetic ? null : legacyToFitTier(p.legacy.tier) }));
  const csvOnly = LABELS_CSV !== undefined && latest.size === 0;
  const fixtureRun = csvOnly || fixtureIds.length > 0;
  ...
  if (decision.fit) {
    ...
    const patch = result.deltas.length
      ...
      : null;
    input = { ...input, search: result, values: result.values, metrics: { before, after }, patch };
