-- Investigators directory: the evidence behind the source chips in one read.
--
-- The directory page used to pull three whole tables into the server on every
-- visit — every RePORTER row with its full raw_json (~8 MB), every source row
-- with its meta blob (~1.7 MB) and all 15k publication rows in 16 sequential
-- pages — to draw three chips per person. On the shared-CPU database that read
-- alone could hit the statement timeout and stall every other request.
--
-- The chips need, per person: every grant (slim: the two dates and the PI
-- roster the role is derived from), how many verified publications there are
-- by identity method, how many are name-only, and the three most recent items
-- the popover shows (two verified, one unverified). That is what this returns.
...
AS $$
  ...
  ranked AS (
    SELECT
      ...
      row_number() OVER (PARTITION BY p.investigator_id, p.identity_status ORDER BY p.publication_date DESC NULLS LAST, p.pmid) AS rn
    FROM public.investigator_publications p
  ...
  recent AS (
    SELECT
      ...
      COALESCE(jsonb_agg(
        ...
        ORDER BY r.publication_date DESC NULLS LAST, r.pmid
